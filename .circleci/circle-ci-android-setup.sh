...
ANDROID_SDK_TARGET_API_LEVEL="22"
# Emulator name
AVD_NAME="testAVD"
...
function getAndroidSDK {
  ...
  if [ ! -e $DEPS ]; then
    ...
    echo y | sdkmanager "system-images;android-$ANDROID_SDK_TARGET_API_LEVEL;google_apis;armeabi-v7a"
...
function createAVD {
  echo y | sdkmanager "system-images;android-$ANDROID_SDK_TARGET_API_LEVEL;default;armeabi-v7a"
  echo no | avdmanager create avd --name $AVD_NAME --force --package "system-images;android-$ANDROID_SDK_TARGET_API_LEVEL;default;armeabi-v7a" --tag default --abi armeabi-v7a
}
...
function launchAVD {
  ...
  #emulator -ports 5760,5761 -prop persist.sys.language=en -prop persist.sys.country=US -avd $AVD_NAME -no-snapshot-load -no-snapshot-save -wipe-data -no-window
}
